// client.js – Updated Client Logic for Raja Rani Game
...
socket.onmessage = (event) => {
  ...
  if (data.type === "your_role") {
    ...
    // Hide role after 3 seconds, allow flip on click
  ...
  if (data.type === "chat") {
    ...
    chatLog.innerHTML += `<p><strong>${data.name}</strong> [${time}]: ${data.text}</p>`;
...
function updatePlayerList(names) {
  document.getElementById("playerList").innerHTML = names.map(n => `<li>${n}</li>`).join('');
}
...
function updateGameCard(data) {
  ...
  const isMyTurn = playerName === data.turnPlayer;
  document.getElementById("guessInput").disabled = !isMyTurn;
  document.querySelector('button[onclick="makeGuess()"]')?.toggleAttribute("disabled", !isMyTurn);
...
function updateScoreboard(scores, id) {
  const box = document.getElementById(id);
  box.innerHTML = `<strong>Total Scores</strong><ul>` +
...
function updateRoundTable(roundTable) {
  ...
  const headers = Object.keys(roundTable[0]);
  box.innerHTML = `<table><thead><tr>` +
    headers.map(h => `<th>${h}</th>`).join('') +
    `</tr></thead><tbody>` +
    roundTable.map(row => `<tr>` + headers.map(h => `<td>${row[h]}</td>`).join('') + `</tr>`).join('') +
    `</tbody></table>`;
}
